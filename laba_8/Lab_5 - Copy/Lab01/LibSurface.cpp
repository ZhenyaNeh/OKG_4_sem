...
void CPlot3D::SortPolygonsByDepth()
{
	for (int i = 0; i < MatrF.size() - 1; i++)
	{
		for (int j = 0; j < MatrF.size() - i - 1; j++)
		{
			double depth1 = (MatrF[j][0](2) + MatrF[j][1](2) + MatrF[j][2](2) + MatrF[j][3](2)) / 4;
			double depth2 = (MatrF[j + 1][0](2) + MatrF[j + 1][1](2) + MatrF[j + 1][2](2) + MatrF[j + 1][3](2)) / 4;

			if (depth1 < depth2)
			{
				CVecMatrix temp = MatrF[j];
				MatrF[j] = MatrF[j + 1];
				MatrF[j + 1] = temp;
			}
		}
